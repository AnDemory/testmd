...
class TicketPdfGenerator {
  ...
  protected function getBackgroundTemplate(array $data, string $webform_id, ?string $domain = NULL): string {
    ...
    if (in_array('domain_hb', $categories) && 'visitor' === $webform_type && !empty(array_filter($data['mijn_congres'] ?? []))) {
      $template_uri .= "-congres";
    }
  ...
  public function save(
    ...
    if (!is_dir($directory) || !is_writable($directory)) {
      throw new \RuntimeException(
        "PDF directory is not writable: $directory"
      );
    }
    ...
    if (!is_file($destination)) {
      throw new \RuntimeException(
        "PDF was not created: $destination"
      );
    }
